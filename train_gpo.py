...
from gpo.gpo_trainer import GPOTrainer
from gpo.gpo_datahelper import get_tokenizer
from utils import *
...
def main(opt):
    ...
    # load adv model
    logging.info(f"Loading adv model from: {opt.adv_model_path}...")
    adv_model = Llama.from_pretrained(opt.adv_model_path, opt, tokenizer)
    adv_model._set_gradient_checkpointing(adv_model.model, opt.gradient_checkpoint)
    ...
    # load adv reference model
    logging.info(f"Loadin adv reference model from: {opt.adv_model_path}...")
    adv_ref_model = Llama.from_pretrained(opt.adv_model_path, opt, tokenizer)
    
    # load llama guard model
    logging.info(f"Loading llama guard model from: {opt.llama_guard_path}...")
    guard_model = AutoModelForCausalLM.from_pretrained(opt.llama_guard_path, opt, tokenizer)

    synchronize_if_distributed()
    trainer = GPOTrainer(opt, policy_model, adv_model, ref_model, adv_ref_model, critic_model, guard_model, accelerator)
    trainer.train()
